...
class Map:
    def __init__(self, width, height, random_seed):
        ...
        for i in range(0, width * height):
            ...
            if xpos == 0:
                ypos += 1
            self.map[i] = Point(0, 'ocean', xpos, ypos)
    ...
    def get_random_neighbor(self, point, only_orthogonal):
        neighbors = self.get_neighbors(point, only_orthogonal)
        return random.choice(neighbors)
    
    def get_random_neighbor_of_type(self, point, biome, only_orthogonal):
        return random.choice(self.get_neighbors_of_type(point, biome, only_orthogonal))
    
    def has_neighbors(self, point, only_orthogonal):
        neighbors = self.get_neighbors(point, only_orthogonal)
        return len(neighbors) > 0
    
    def has_neighbors_of_type(self, point, biome, only_orthogonal):
        neighbors = self.get_neighbors_or_type(point, biome, only_orthogonal)
        return len(neighbors) > 0
    
    def get_neighbors_of_type(self, point, biome, only_orthogonal):
        neighbors = self.get_neighbors(point, only_orthogonal)
        result = []
        for i in range(0, len(neighbors)):
